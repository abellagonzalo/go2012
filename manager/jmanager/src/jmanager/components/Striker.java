 ...
 * @author paco
 */
public class Striker extends javax.swing.JPanel {
	...
	private void initComponents() {
		...
		org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance().getContext().getResourceMap(Striker.class);
	...
	private void ActivationCheckBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ActivationCheckBoxActionPerformed
		if (ActivationCheckBox.isSelected()) {
			JmanagerView.root.getConnection().schedulerPrx.run("Striker");
			//dataResp resp = JmanagerView.root.getConnection().send("run:Striker:");

			//try {
			//	Thread.sleep(3000);
			//} catch (InterruptedException ex) {
			//	Logger.getLogger(Striker.class.getName()).log(Level.SEVERE, null, ex);
			//}

		} else {
			JmanagerView.root.getConnection().schedulerPrx.stop("Striker");
			//dataResp resp = JmanagerView.root.getConnection().send("pause:Striker:");
			//printResp(resp);
		}
