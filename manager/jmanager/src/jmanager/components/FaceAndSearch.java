 ...
 * @author paco
 */
public class FaceAndSearch extends javax.swing.JPanel {

	/** Creates new form ComponentGTLocalization */
	public FaceAndSearch() {
		initComponents();
	}
	...
	private void initComponents() {
		...
		org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance().getContext().getResourceMap(FaceAndSearch.class);
	...
	private void ActivationCheckBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ActivationCheckBoxActionPerformed
		if (ActivationCheckBox.isSelected()) {
			JmanagerView.root.getConnection().schedulerPrx.run("FaceAndSearch");
			//dataResp resp = JmanagerView.root.getConnection().send("run:FaceAndSearch:");

			//try {
			//	Thread.sleep(3000);
			//} catch (InterruptedException ex) {
			//	Logger.getLogger(FaceAndSearch.class.getName()).log(Level.SEVERE, null, ex);
			//}

		} else {
			JmanagerView.root.getConnection().schedulerPrx.stop("FaceAndSearch");
			//dataResp resp = JmanagerView.root.getConnection().send("pause:FaceAndSearch:");
			//printResp(resp);
		}
