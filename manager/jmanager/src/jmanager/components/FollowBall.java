 ...
 * @author paco
 */
public class FollowBall extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(jmanager.JmanagerApp.class).getContext().getResourceMap(FollowBall.class);
    ...
    private void ActivationCheckBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ActivationCheckBoxActionPerformed
        if (ActivationCheckBox.isSelected()) {
        	JmanagerView.root.getConnection().schedulerPrx.run("Goalie");
        	//dataResp resp = JmanagerView.root.getConnection().send("run:FollowBall:");
            
            //try {
            //    Thread.sleep(3000);
            //} catch (InterruptedException ex) {
            //    Logger.getLogger(FollowBall.class.getName()).log(Level.SEVERE, null, ex);
            //}

        } else {
        	JmanagerView.root.getConnection().schedulerPrx.stop("Goalie");
        	//dataResp resp = JmanagerView.root.getConnection().send("pause:FollowBall:");
            //printResp(resp);
        }
