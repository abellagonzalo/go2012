...
import jmanager.VICODE.Builder;
 ...
 */
public class InfoLoaderContentHandler implements ContentHandler {
	...
	public InfoLoaderContentHandler() {
	}
	...
	public void startElement(String namespaceURI, String localName,
		...
		if (localName.equals("conf")) {
			int auto = 0;

			for (int i = 0; i < atts.getLength(); i++) {
				if (atts.getLocalName(i).equals("autorestart")) {
					auto = Integer.parseInt(atts.getValue(i).toString());
				}
			}
			gui.setAutoRestart(auto == 1);

			Builder.getInstance().setAutoRestart(auto == 1);
		}
	}
