 ...
 */
public class StatesLoaderContentHandler implements ContentHandler {
	...
	public StatesLoaderContentHandler() {
	}
	...
	public void startElement(String namespaceURI, String localName,
		...
		if (localName.equals("state")) {
			...
			if (id.equals("initial")) {
				nowState = gui.getInitialState();
				nowState.setXYpos(x, y);
				nowState.centerIdText();
			} else {
				nowState = gui.addState(id);
				nowState.setXYpos(x, y);
				nowState.centerIdText();
				//nowstate = myState;
			}

		} else if (localName.equals("modulation")) {
			String id = new String();
			for (int i = 0; i < atts.getLength(); i++) {
				if (atts.getLocalName(i).equals("id")) {
					id = atts.getValue(i).toString();
			...
			Component comp = gui.getComponent(id);

			if (comp != null) {
				nowState.addModulation(nowState, comp);
			} else {
				System.err.println("Modulación desde " + nowState.getId() + ": Componente " + comp + "no encontrada");
			}
			...
			nowState.addRequirement(gui.getComponent(id), nowState);
		}
	}
