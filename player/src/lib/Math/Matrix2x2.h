...
#include "Vector2D_BH.h"

/** This class represents a 2x2-matrix */
template <class V = double> class Matrix2x2 {
...
public:
  ...
  Matrix2x2<V>()
  {
    c[0]=Vector2D_BH<V>(1,0);
    c[1]=Vector2D_BH<V>(0,1);
  }
  ...
  */
  Matrix2x2<V>(const Vector2D_BH<V>& c0,const Vector2D_BH<V>& c1)
  {
    c[0]=c0;
    c[1]=c1;
  }
  ...
  */
  ...
  \return A reference to a new vector containing the result
    of the calculation.
  */
  Vector2D_BH<V> operator*(const Vector2D_BH<V>& vector) const
  {
  ...
  */
  Matrix2x2<V> operator+(const Matrix2x2<V>& other) const
  {
    return Matrix2x2<V>(Vector2D_BH<V>(c[0].x + other.c[0].x,c[0].y + other.c[0].y),
                        Vector2D_BH<V>(c[1].x + other.c[1].x,c[1].y + other.c[1].y));
  }
  ...
  */
  Matrix2x2<V> invert() const
  {
    ...
    if(fabs(factor) < 0.00000000000000001)
      ...
      factor = 1.0/factor;
    return Matrix2x2<V>(Vector2D_BH<V>(factor * c[1].y,-factor * c[0].y),
                        Vector2D_BH<V>(-factor * c[1].x,factor * c[0].x));
  }
  ...
  */
  ...
  {
    return Matrix2x2<V>(Vector2D_BH<V>(c[0].x,c[1].x),
                        Vector2D_BH<V>(c[0].y,c[1].y));
  }
