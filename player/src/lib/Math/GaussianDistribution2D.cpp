...
{
  Vector2D_BH<double> diff(mean - other.mean);
  Matrix2x2<double> cov(covariance + other.covariance);
  return diff*(cov.invert()*diff);
}

double GaussianDistribution2D::probabilityAt(const Vector2D_BH<double>& pos) const
{
  Vector2D_BH<double> diff(pos - mean);
...
void GaussianDistribution2D::merge(const GaussianDistribution2D& other)
{
  ...
  double w1(p1/pSum);
  double w2(p2/pSum);
  Vector2D_BH<double> newMean(mean*w1 + other.mean*w2);
  Vector2D_BH<double> diffMean(mean - other.mean);
...
Vector2D_BH<double> GaussianDistribution2D::rand() const
{
  Matrix2x2<double> L;
  choleskyDecomposition(covariance, L, 1E-9*(covariance.trace()));
  Vector2D_BH<double> xRaw(randomGauss(), randomGauss());
  return L * xRaw;
}
...
void GaussianDistribution2D::getEigenVectorsAndEigenValues(Vector2D_BH<double>& eVec1, Vector2D_BH<double>& eVec2,
